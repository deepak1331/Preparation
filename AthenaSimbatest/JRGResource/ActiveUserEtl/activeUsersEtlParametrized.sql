...
WITH transactions AS (
	...
	where date(createstamp - interval '5' hour) >= date_trunc('month', cast('startDate' as date))
	and date(createstamp - interval '5' hour) < cast('endDate' as date)
...
transaction_summary AS (
	...
	and lucktastic_day = cast('startDate' as date)
	group by 1, 2, 3, 4, 5
	-- monthly totals
	union
	select
...
raw_events AS (
	...
	where (
		(year = cast(year(cast('startDate' as date)) as varchar)
		and month = lpad(cast(month(cast('startDate' as date)) as varchar), 2, '0'))
		or 
		(year = cast(year(cast('endDate' as date)) as varchar)
		and month = lpad(cast(month(cast('endDate' as date)) as varchar), 2, '0')
		and day = lpad(cast(day(cast('endDate' as date)) as varchar), 2, '0'))
		)
	and date(from_unixtime(timestamp/1000) at time zone 'America/New_York' - interval '5' hour) >= date_trunc('month', cast('startDate' as date))
	and date(from_unixtime(timestamp/1000) at time zone 'America/New_York' - interval '5' hour) < cast('endDate' as date)
